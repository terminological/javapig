...
final class AptUtils {
			...
			public Void visitWildcard(WildcardType t, Void p) {
				...
				if (type instanceof PrimitiveType) {
					...
					if (type == null) {
						...
						typeToString(type, result, innerClassSeparator);
					}
				}
	...
	public static boolean isAnnotationPresent(Class<?> annotationType, Element element) {	
		for (AnnotationMirror annotation : element.getAnnotationMirrors()) {
			if (rawTypeToString(annotation.getAnnotationType(), '$')
					.equals(annotationType.getName())) {
				return true;
	...
	public static Map<String, Map<String, Object>> getAllAnnotations(Element element) {
		...
		for (AnnotationMirror annotation : element.getAnnotationMirrors()) {
			...
			out.put(rawTypeToString(annotation.getAnnotationType(), '$'),result);
		}
		return out;
	}
	...
	public static String firstParameter(TypeMirror asType) {
		if (asType instanceof DeclaredType) {
		List<? extends TypeMirror> typeArguments = ((DeclaredType) asType).getTypeArguments();
		if (typeArguments.size() == 0) return null;
			return typeToString(typeArguments.get(0));
		} else if (asType instanceof WildcardType) {
			return firstParameter(((WildcardType) asType).getExtendsBound());
		} else {
			return null;
		}
	}
